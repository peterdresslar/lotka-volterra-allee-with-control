...
# --- ODEs and helpers ---#
def base_lv_ode(
    s: float, w: float, alpha: float, beta: float, gamma: float, delta: float
) -> tuple[float, float]:
...
def calculate_phase_space_vectors(
    alpha: float,
    beta: float,
    gamma: float,
    delta: float,
    ...
    w0: float,
    t_max: float = 50.0,
) -> np.ndarray:  # removed dt parameter
    reset_events()
...
def add_example_2_sidebar() -> None:
    ...
    st.sidebar.number_input(
        "K", key="K", value=200, min_value=0, max_value=1000, step=1
    )
...
def render_example_1() -> None:
    ...
    solution = solve_ivp(
        base_lv_ode_ivp,  # base system of equations
        [0.0, T],  # time range
    ...
    st.caption("Figure 1")
    st.line_chart(
        solution_df, x_label="Time", y_label="Population Density"
    )  # plot the solution
    ...
    st.caption("Figure 2")
    s_null = (
        alt.Chart(pd.DataFrame({"x": [gamma / delta]}))
        .mark_rule(color="red", strokeDash=[4, 4])
        .encode(x="x:Q")
    )
    w_null = (
        alt.Chart(pd.DataFrame({"y": [alpha / beta]}))
        .mark_rule(color="blue", strokeDash=[4, 4])
        .encode(y="y:Q")
    )
    eq_pt = alt.Chart(
        pd.DataFrame({"Sheep": [gamma / delta], "Wolves": [alpha / beta]})
    ).mark_point(color="black", size=60)
...
def render_example_2() -> None:
    ...
    st.caption("Figure 3")
    st.line_chart(
        lv_star_solution_df, x_label="Time", y_label="Population Density"
    )  # plot the solution
...
def render_footer() -> None:
    ...
    st.page_link(
        "pages/lotka_volterra_star_stability.py",
        label="Next: LV* with stability analysis",
        icon="➡️",
    )
